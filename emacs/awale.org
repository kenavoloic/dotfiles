...
(use-package flycheck
  ...
  (global-flycheck-mode t)
  :config
  ;; Utilise executable-find (compatible Emacs 28 et +)
  (setq flycheck-executable-find
        (lambda (cmd)
          (or (executable-find cmd)
              (let ((uv-env (or (getenv "VIRTUAL_ENV") (getenv "UV_ENV"))))
                (when uv-env
                  (let ((ruff-path (expand-file-name (concat uv-env "/bin/" cmd))))
                    (if (file-exists-p ruff-path) ruff-path nil)))))))
  ;; Force le format de sortie valide pour Ruff
  (setq flycheck-ruff-args '("--output-format=text")))
#+end_src
...
;;   (global-flycheck-mode t)
;;   :config
;;   ;; Force Flycheck à utiliser le Ruff de l'environnement uv
;;   (setq flycheck-executable-find
;;         (lambda (cmd)
;;           (or (locate-file-in-path cmd)
;;               (let ((uv-env (or (getenv "VIRTUAL_ENV") (getenv "UV_ENV"))))
;;                 (when uv-env
;;                   (expand-file-name (concat uv-env "/bin/" cmd)))))))
;;   ;; Corriger le format de sortie invalide
;;   (setq flycheck-ruff-args '("--output-format=text")))
...
(use-package org-tempo
  ...
  :config (setq org-structure-template-alist
		'(("el" . "src emacs-lisp")
		  ...
		  ("ja"  . "src java\n")
		  ("sc"  . "src C")
		  ("sl"  . "src lilypond :file xxx.pdf")
		  ("sc2"  . "src C :exports results :tangle sources/\n")
		  ("snq"  . "src sql :eval no\n")
		  ("su"  . "src plantuml :file images/image.png\n")
		  ("sx" . "src xml\n")
		  ("sq" . "src sql\n"))))
#+end_src
...
https://alhassy.com/emacs.d/index.html
...
(use-package lsp-mode
  ...
  :defer t
  :init (setq lsp-keymap-prefix "C-c l"
              lsp-diagnostics-provider :flymake) ; ou :flycheck
  :config
  (lsp-enable-which-key-integration t)
  ;; Utilise executable-find pour trouver ruff-lsp
  (setq lsp-ruff-executable
        (lambda ()
          (or (executable-find "ruff-lsp")
              (let ((uv-env (or (getenv "VIRTUAL_ENV") (getenv "UV_ENV"))))
                (when uv-env
                  (let ((ruff-lsp-path (expand-file-name (concat uv-env "/bin/ruff-lsp"))))
                    (if (file-exists-p ruff-lsp-path) ruff-lsp-path nil)))))))
  :commands (lsp lsp-deferred))
#+end_src
...
;;   :defer t
;;   :init (setq lsp-keymap-prefix "C-c l"
;;               lsp-diagnostics-provider :flymake) ; ou :flycheck si tu préfères
;;   :config
;;   (lsp-enable-which-key-integration t)
;;   ;; Pointer vers le Ruff de l'environnement uv
;;   (setq lsp-ruff-executable
;;         (lambda ()
;;           (or (locate-file-in-path "ruff")
;;               (let ((uv-env (or (getenv "VIRTUAL_ENV") (getenv "UV_ENV"))))
;;                 (when uv-env
;;                   (expand-file-name (concat uv-env "/bin/ruff")))))))
;;   :commands (lsp lsp-deferred))
#+end_src
...
;; désactive depuis le passage à uv
;; 29.04.2026
#+end_src
...
* noxml-fold
https://github.com/paddymcall/noXML-fold/tree/master
#+begin_src emacs-lisp
(add-hook 'nxml-mode-hook (lambda () (require 'noxml-fold)))
#+end_src

#+RESULTS:
| #[nil ((require 'noxml-fold)) nil] |
...
#+RESULTS:
| pipenv-mode | lsp-deferred | doom-modeline-env-setup-python |
...
#+RESULTS:
| uv-mode-auto-activate-hook | pipenv-mode | lsp-deferred | doom-modeline-env-setup-python |
